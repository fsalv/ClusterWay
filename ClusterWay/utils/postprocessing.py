...
def wp_post_processing(mask,wp,angle_est_iter=30,rng=None):
    """
...
def find_point(mask,p1=(400,400), rng=None):
    """
        Find a suitable point out where mask is 1 starting from p1
    """
    p1+=random_displ(5,shape=(2), rng=rng) 
...
def minimize_edges(mask,p1=(400,400),l=300,rng=None):
    """
...
def estimate_row_angle(img,verbose=False,rng=None):
    ...
    if linesP is None:   #use the iterative random approach
        ...
        return estimate_row_angle_iterative(img,rng=rng)
...
def estimate_row_angle_iterative(mask,iterations=30,rng=None):
    """
    ...
    for _ in range(iterations):
        p1 = find_point(mask,mask_center,rng=rng)
        row_angle.append(minimize_edges(mask,p1,l), rng=rng)
...
def cluster_wp(wp,eps=40,min_samples=2,verbose=False):
    ...
    """
    min_samples = min(min_samples, len(wp)//2)
    ...
    if max(count[lab!=-1])>0.8*len(wp):  # unbalanced clustering
        if verbose:
...
def order_cluster(wp, row_normal):
    """
        Project cluster along row normal direction
    """
