...
def create_mask(points,borders,H=800,W=800,radius=[3,4],hole_prob=0.5,hole_dim=[4,6],hole_frame=12,max_curved_displ=50,curved=False):
    """
    ...
    rows = []
    if curved:
        rows = get_curve_lines(points, borders, max_curved_displ)
        if rows is None:  #if max iteration, not possible to get curved in this conditions
    ...
    return mask.astype("bool"), row_lines, True
...
def get_curve_lines(points, borders, max_displ=50):
    ...
    while not done:
        if i>100: #max iterations
            return None
        disp = np.random.randint(- max_displ, max_displ, (2))
        if (disp == [0,0]).all(): # no displacement, resample
            continue
        p += disp
